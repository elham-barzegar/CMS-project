...
const Modal = ({Trigger, children, title, onSubmit}) => {
    ...
    return (  <>
            <div className="min-w-max min-h-max cursor-pointer" onClick={openModal}>{Trigger}</div>

            <div id="modal-screen" className={clsx("w-dvw h-dvh right-0 top-0 fixed z-10 bg-black/30 flex-center duration-300",
                ...
                <div className="w-[500px] rounded-lg overflow-hidden bg-white drop-shadow-2xl/30 max-h-[90dvh] overflow-y-auto">
                    <div className="flex items-center justify-between px-4 h-16">
                        ...
                        <button onClick={closeModal} className="text-2xl! text-zinc-500 cursor-pointer">
                            <BiX  />
                        </button>
                    </div>
